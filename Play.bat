...
rem Accept 127.0.0.1 (single-player) or the multiplayer advertised host from
rem evejs.config.local.json / EVEJS_GAME_SERVER_HOST when hosting for friends.
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ini=$env:EVEJS_START_INI; $server=$null; $crypto=$null; foreach($line in Get-Content -LiteralPath $ini){ if($line -match '^\s*(server|serverip)\s*=\s*(.+?)\s*$'){ $server=$Matches[2].Trim() }; if($line -match '^\s*cryptoPack\s*=\s*(.+?)\s*$'){ $crypto=$Matches[1].Trim() } }; $allowed=@('127.0.0.1','localhost'); if($env:EVEJS_GAME_SERVER_HOST){ $allowed += $env:EVEJS_GAME_SERVER_HOST.Trim() }; $cfg=Join-Path $env:EVEJS_REPO_ROOT 'evejs.config.local.json'; if(Test-Path -LiteralPath $cfg){ foreach($line in Get-Content -LiteralPath $cfg){ if($line -match '^\s*\"gameServerHost\"\s*:\s*\"([^\"]+)\"'){ $allowed += $Matches[1].Trim() } } }; $allowed = $allowed | Where-Object { $_ } | Select-Object -Unique; if($allowed -notcontains $server){ exit 3 }; if($crypto -ne 'Placebo'){ exit 4 }; exit 0"
set "EVEJS_START_INI_EXIT=%errorlevel%"
if "%EVEJS_START_INI_EXIT%"=="3" (
  echo.
  echo   [ERROR] start.ini is not pointed at this EvEJS server.
  echo       For local play run tools\ClientSETUP\StartClientSetup.bat Step 5.
  echo       For multiplayer set start.ini server= to the host IP, or use the
  echo       PlayerConnect bundle Connect.bat.
  exit /b 1
)
...
call :ResolvePlayServerHost
echo   Checking EvEJS server at %EVEJS_PLAY_SERVER_HOST%...
...
echo.
echo       Expected:
echo         Game TCP: %EVEJS_PLAY_SERVER_HOST%:26000
echo         Proxy:    %EVEJS_PROXY_URL%health
exit /b 1

:ResolvePlayServerHost
set "EVEJS_PLAY_SERVER_HOST=127.0.0.1"
if defined EVEJS_GAME_SERVER_HOST if not "%EVEJS_GAME_SERVER_HOST%"=="" set "EVEJS_PLAY_SERVER_HOST=%EVEJS_GAME_SERVER_HOST%"
if exist "%EVEJS_REPO_ROOT%\evejs.config.local.json" (
  for /f "usebackq delims=" %%H in (`powershell -NoProfile -ExecutionPolicy Bypass -Command "$cfg=Join-Path $env:EVEJS_REPO_ROOT 'evejs.config.local.json'; if(Test-Path -LiteralPath $cfg){ foreach($line in Get-Content -LiteralPath $cfg){ if($line -match '^\s*\"gameServerHost\"\s*:\s*\"([^\"]+)\"'){ Write-Output $Matches[1].Trim(); break } } }"`) do set "EVEJS_PLAY_SERVER_HOST=%%H"
)
if not defined EVEJS_PLAY_SERVER_HOST set "EVEJS_PLAY_SERVER_HOST=127.0.0.1"
if "%EVEJS_PLAY_SERVER_HOST%"=="" set "EVEJS_PLAY_SERVER_HOST=127.0.0.1"
if /I "%EVEJS_PLAY_SERVER_HOST%"=="localhost" set "EVEJS_PLAY_SERVER_HOST=127.0.0.1"
exit /b 0

:CheckGameServerPort
powershell -NoProfile -ExecutionPolicy Bypass -Command "$hostName=$env:EVEJS_PLAY_SERVER_HOST; if(-not $hostName){ $hostName='127.0.0.1' }; $client=[Net.Sockets.TcpClient]::new(); try { $iar=$client.BeginConnect($hostName,26000,$null,$null); if(-not $iar.AsyncWaitHandle.WaitOne(750,$false)){ exit 1 }; $client.EndConnect($iar); exit 0 } catch { exit 1 } finally { $client.Close() }" >nul 2>&1
exit /b %errorlevel%
